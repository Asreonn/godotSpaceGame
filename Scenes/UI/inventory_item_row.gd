...
func configure(ui_ref: Node, new_item_id: String, new_is_ship: bool, p_name: String, color: Color) -> void:
	_ui = ui_ref
	item_id = new_item_id
	is_ship = new_is_ship
	display_name = p_name
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if _ui and _ui.has_method("on_item_left_press"):
				_ui.on_item_left_press(item_id, is_ship)
				accept_event()
		elif event.button_index == MOUSE_BUTTON_RIGHT:
			if _ui and _ui.has_method("on_item_right_press"):
